...
router.post('/api/closed-days', isAuthenticated, isAdmin, async (req, res) => {
    ...
    try {
        await db.promise().query(`INSERT INTO closed_days (start_date, end_date, reason) VALUES (?, ?, ?)`, [start_date, end_date, reason]);
...
router.delete('/api/bookings', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        await db.promise().query(`DELETE FROM desk_bookings WHERE id=?`, [bookingId]);
        io.emit('bookingUpdate');
