...
router.get('/api/checkAuth', (req, res) => {
    if (req.session.isAuthenticated) {
        res.status(200).json({ isAuthenticated: true, name: req.session.user.name, is_admin: req.session.user.is_admin, user_id: req.session.user.user_id, department_id: req.session.user.department_id, is_fulltime: req.session.user.is_fulltime });
    } else {
