...
router.get('/api/office/:year/:week', isAuthenticated, async (req, res) => {
    ...
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN users.is_vegetarian = 1 THEN desk_bookings.user_id END) AS vegetarianCount,
        COUNT(DISTINCT CASE WHEN users.is_vegan = 1 THEN desk_bookings.user_id END) AS veganCount
    FROM 
    ...
    LEFT JOIN 
        users ON desk_bookings.user_id = users.id
    WHERE 
